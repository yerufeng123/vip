...
$(document).ready(function(e) {
	...
	slide.SlideY = function(_dir){
		if(page._index == 3&&_dir){
			clearInterval(page.subtitle);
		}
		page_move(_dir);
	}
	//下一页跳转
...
function page_move(_index,sign){ 
	...
	if(page._index == 3){
		page.subtitle = setInterval(function(){
				video_play();
			},3000)
	}
	//是否移除原来动画结果
...
/*字幕播放*/
function video_play(){
	var _doms = $('.p4_detail .p4_li');
	var _len = _doms.length;
	var pre = $('.p4_detail .word_now').index();
	var index = (pre+1+_len)%_len;
	var next = (pre+2+_len)%_len;
	_doms.removeClass('word_now word_pre word_next');
	_doms.eq(pre).addClass('word_pre');
	_doms.eq(index).addClass('word_now');
	_doms.eq(next).addClass('word_next');

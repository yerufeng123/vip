...
/**
 * This is the model class for table "{{guanzhi_administrator}}".
 *
 * The followings are the available columns in table '{{guanzhi_administrator}}':
 * @property integer $id
 ...
 */
class GuanzhiAdministrator extends CActiveRecord
{
	/**
	...
	public static function model($className=__CLASS__)
	{
        ...
        public function getAdminInfo($id){
            $sql="select * from vip_guanzhi_administrator where id=".$id;
        ...
        public function getAdminInfo2($username){
            $sql="select * from vip_guanzhi_administrator where username='{$username}'";
            $admin=Yii::app()->db->createCommand($sql)->queryAll();
            $admin1=  empty($admin)? array() : $admin[0];
            return $admin1;
        }
        ...
        public function checkAdmin($id,$role,$username){
            $sql="select id from vip_guanzhi_administrator where id={$id} and role={$role} and username='{$username}'";
            $admin=Yii::app()->db->createCommand($sql)->queryAll();
            if(empty($admin)){
                return false;
            }

...
//管理员显示
    public function actionIndex() {
        ...
        if(!empty($where)){
            $sql = "select * from vip_guanzhi_administrator where {$where} order by ctime desc limit {$startnum},{$pernum} ";
        }else{
            $sql = "select * from vip_guanzhi_administrator order by ctime desc limit {$startnum},{$pernum}";
        }
        $data = Yii::app()->db->createCommand($sql)->queryAll();
        if(!empty($where)){
             $sql2 = "select count(id) as num from vip_guanzhi_administrator where {$where}";
        }else{
             $sql2 = "select count(id) as num from vip_guanzhi_administrator";
        }
    ...
    public function actionAdmin_add() {
        if (empty($_POST)) {//非异步
            ...
            $sql="select id from vip_guanzhi_administrator where username='{$_POST['username']}'";
            $data=  Yii::app()->db->createCommand($sql)->queryAll();
            if(!empty($data)){
                echo '该账号已被注册！';die;
            }
    ...
    public function actionAdmin_editor() {
        if (empty($_POST)) {
            ...
            $sql="select * from vip_guanzhi_administrator where id='{$_POST['id']}'";
            $data=  Yii::app()->db->createCommand($sql)->queryAll();
            ...
            $sql="select id from vip_guanzhi_administrator where username='{$_POST['username']}' and id <> '{$_POST['id']}'";
            $data=  Yii::app()->db->createCommand($sql)->queryAll();
            if(!empty($data)){
                echo '该账号已被注册！';die;
            }
            $admin= guanzhiAdministrator::model();
            $admin->id=$_POST['id'];
            $admin->username=$_POST['username'];
            $admin->name=$_POST['name'];
            $admin->role=$_POST['role'];

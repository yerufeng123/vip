...
class BaseController extends Controller {
    ...
    public function __construct($id, $module = null) {
        ...
        $this->administrator=new QizhengAdministrator();
        $this->login();
        $admin=$this->administrator->getAdminInfo($_SESSION['admin_id']);//获取当前用户的信息
        $this->user_name=$admin['name'];
        
        $sql='select accessnum from vip_qizheng_web where id=1';
        $accessnum=  Yii::app()->db->createCommand($sql)->queryRow();
        $this->accessnum=$accessnum['accessnum'];

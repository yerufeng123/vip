...
/**
 * This is the model class for table "{{qizheng_administrator}}".
 *
 * The followings are the available columns in table '{{qizheng_administrator}}':
 * @property integer $id
 ...
 */
class QizhengAdministrator extends CActiveRecord
{
	/**
	...
	public static function model($className=__CLASS__)
	{
        ...
        public function getAdminInfo($id){
            $sql="select * from vip_qizheng_administrator where id=".$id;
        ...
        public function getAdminInfo2($username){
            $sql="select * from vip_qizheng_administrator where username='{$username}'";
            $admin=Yii::app()->db->createCommand($sql)->queryAll();
            $admin1=  empty($admin)? array() : $admin[0];
            return $admin1;
        }
        ...
        public function checkAdmin($id,$role,$username){
            $sql="select id from vip_qizheng_administrator where id={$id} and role={$role} and username='{$username}'";
            $admin=Yii::app()->db->createCommand($sql)->queryAll();
            if(empty($admin)){
                return false;
            }

    ...
    <form id="myForm" name="myForm"  enctype="multipart/form-data">
        <div style="margin:auto;width: 600px;border: 10px solid #ffffff;">
            ...
            类型:<select name="type" class="select25">
                <option value="1" <?php if ($list['type'] == '1') {
    echo 'selected';
} ?>>优秀队长</option>   
                <option value="2" <?php if ($list['type'] == '2') {
    echo 'selected';
} ?>>优秀队员</option>
                <option value="3" <?php if ($list['type'] == '3') {
    echo 'selected';
} ?>>优秀内勤</option> 
            </select><font color="red">*必填</font><br />
            姓名:<input type="text" name="name" value="<?php echo $list['name'] ?>" class="input25" maxlength="125"/><font color="red">*必填</font><br/>
            描述:<textarea name="content" class="textarea100" ><?php echo $list['content'] ?></textarea><font color="red">*必填</font><br />
            <div class="clearbar5"></div>
            图片:<font color="red">*必填(140*110)</font><input id="file_upload1"  type="file" multiple="true">
            <div id="tu_photo1">
                <?php
                if (!empty($list['pic'])) {
                    echo '<img src="/' . $list['pic'] . '" width="140" height="110"/>';
                    echo '<input type="button" class="buttons" value="删除" onclick="delpic(this, \'little_pic\')"/>';
                }
                ?>
            </div><br /><br /><br /><br /><br /><br />
            <input type="hidden" name="little_pic" id="little_pic" value="<?php echo $list['pic']?>"/>
        ...
        <div class="clearbar10"></div>
        <input type='hidden' id='controlPath' value='/jinganshun/interact/elect_editor'/>
    </form>
</div>

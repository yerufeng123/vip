...
//管理员显示
    public function actionIndex() {
        ...
        if(!empty($where)){
            $sql = "select * from vip_jinganshun_user where {$where} order by ctime desc limit {$startnum},{$pernum} ";
        }else{
            $sql = "select * from vip_jinganshun_user order by ctime desc limit {$startnum},{$pernum}";
        }
        $data = Yii::app()->db->createCommand($sql)->queryAll();
        if(!empty($where)){
             $sql2 = "select count(id) as num from vip_jinganshun_user where {$where}";
        }else{
             $sql2 = "select count(id) as num from vip_jinganshun_user";
        }
    ...
    public function actionUser_add() {
        if (empty($_POST)) {//非异步
            ...
            $sql="select id from vip_jinganshun_user where username='{$_POST['username']}'";
            $data=  Yii::app()->db->createCommand($sql)->queryAll();
            if(!empty($data)){
                echo '该账号已被注册！';die;
            }
            ...
            $user->starttime=$_POST['starttime'];
            $user->endtime=$_POST['endtime'];
            $user->ctime=time();
            if($user->save()>0){
                //注册成功
            }else{
    ...
    public function actionUser_editor() {
        if (empty($_POST)) {
            ...
            $sql="select * from vip_jinganshun_user where id='{$_POST['id']}'";
            $data=  Yii::app()->db->createCommand($sql)->queryAll();

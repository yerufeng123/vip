...
class Wechatg {
    ...
    const MEDIA_UPLOADNEWS_URL = '/media/uploadnews?'; //过期的连接 高祥栋 2015-4-6
    const MATERIAL_ADDNEWS_URL = '/material/add_news?'; //新增永久图文素材 高祥栋 2015-4-6
    const MATERIAL_UPDATENEWS_URL = '/material/update_news?'; //修改永久图文素材 高祥栋 2015-4-6
    const MATERIAL_ADDMATERIAL_URL = '/material/add_material?'; //新增永久其他素材 高祥栋 2015-4-6
    ...
    const UPLOAD_MEDIA_URL = 'http://file.api.weixin.qq.com/cgi-bin'; //（作废了好像 高祥栋 2015-4-6）
    const UPLOAD_MATERIAL_URL = 'http://api.weixin.qq.com/cgi-bin'; //新增上传永久其他素材前缀 高祥栋 2015-4-6
    const MEDIA_UPLOAD_URL = '/media/upload?';
    const MEDIA_GET_URL = '/media/get?';
    const MATERIAL_GETMATERIAL_URL = '/material/get_material?'; //新增获取永久素材 高祥栋 2015-4-6
    const MATERIAL_DELMATERIAL_URL = '/material/del_material?'; //新增删除永久素材 高祥栋 2015-4-6
    ...
    const CARD_MEETINGTICKET_UPDATEUSER = '/card/meetingticket/updateuser?'; //更新会议门票(高祥栋 2015-4-4)
    const CARD_UPLOAD_LOGO = '/media/uploadimg?'; //卡券上传商家logo
    ...
    private $jsapi_ticket;//js接口所用ticket
    private $card_ticket;//卡券所用ticket
    private $user_token;
    private $partnerid;
    private $partnerkey;
    private $paysignkey;
    ...
    /**
     * 获取接收卡包事件推送（高祥栋 2015-4-2 add）
     */
    public function getRevCardEvent() {
        if (isset($this->_receive['Event'])) {
            $array['event'] = $this->_receive['Event'];
        }
        if (isset($this->_receive['CardId'])) {//审核、领取、删除
            $array['cardid'] = $this->_receive['CardId'];
        }
        if (isset($this->_receive['FriendUserName'])) {//领取
            $array['fopenid'] = $this->_receive['FriendUserName']; //赠送方openid
        }
        if (isset($this->_receive['IsGiveByFriend'])) {//领取
            $array['isgive'] = $this->_receive['IsGiveByFriend']; //是否转增
        }
        if (isset($this->_receive['UserCardCode'])) {//领取、删除
            $array['ucode'] = $this->_receive['UserCardCode']; //code序列号
        }
        if (isset($this->_receive['OuterId'])) {//领取
            $array['outerid'] = $this->_receive['OuterId']; //领取场景值
        }
    ...
    /**
     * 获取卡券事件推送 - 卡卷审核是否通过(建议使用 getRevCardEvent()方法)
    ...
    /**
     * 获取卡券事件推送 - 领取卡券 (建议使用 getRevCardEvent()方法)
    ...
    /**
     * 获取卡券事件推送 - 删除卡券(建议使用 getRevCardEvent()方法)
     ...
     *  	"0"=>array(
     *  		'Title'=>'msg title',
     *  		'Description'=>'summary text',//当有多个图文时，图文描述不显示
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    private function http_post($url, $param, $post_file = false) {
        ...
        $aStatus = curl_getinfo($oCurl);
        curl_close($oCurl);
        file_put_contents(_STATICPATH_.'wechat_token_all.txt', $sContent."#####\r\n",FILE_APPEND);
        if (intval($aStatus["http_code"]) == 200) {
            return $sContent;
        } else {
            file_put_contents(_STATICPATH_.'wechat_token_err.txt', $sContent."#####\r\n",FILE_APPEND);
            return false;
        }
    }
    ...
    protected function setCache($cachename, $value, $expired) {
        ...
        file_put_contents(_STATICPATH_ . 'weixincache/' . $cachename, json_encode(array('value' => $value, 'expired' => $expired, 'savetime' => time())));
        return false;
    }
    ...
    protected function getCache($cachename) {
        ...
        if (file_exists(_STATICPATH_ . 'weixincache/' . $cachename)) {
            $cache = json_decode(file_get_contents(_STATICPATH_ . 'weixincache/' . $cachename), true);
            if ($cache['expired'] + $cache['savetime'] > time()) {
                return $cache['value'];
            } else {
                file_put_contents(_STATICPATH_ . 'weixincache/log.txt', json_encode($cache) . '=====' . time(), FILE_APPEND);
            }
        }
        return false;
    }
    ...
    protected function removeCache($cachename) {
        ...
        if (file_exists(_STATICPATH_ . 'weixincache/' . $cachename)) {
            return unlink(_STATICPATH_ . 'weixincache/' . $cachename);
        }
        return false;
    }
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '') {
        ...
        if ($token) { //手动指定token，优先使用
         ...
         $this->access_token=WXAccessToken::getToken($appid, $appsecret);
     ...
     */
    public function getJsTicket($appid = '',$appsecret = '', $jsapi_ticket = '') {
        ...
        $this->jsapi_ticket =  WXAccessToken::getApiTicket($appid, $appsecret);
        return $this->jsapi_ticket;
    }

    /**
     * 上边那个不能用了，试试这个（卡券api调用）
     * 获取卡券API TICKET
     * @param string $appid 用于多个appid时使用,可空
     * @param string $card_ticket 手动指定card_ticket，非必要情况不建议用
     */
    public function getCardTicket($appid = '', $card_ticket = '') {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        if (!$appid)
            $appid = $this->appid;
        if ($card_ticket) { //手动指定token，优先使用
            $this->card_ticket = $card_ticket;
            return $this->card_ticket;
        }
        $authname = 'wechat_card_ticket' . $appid;
        ...
        if ($result) {
            ...
            $this->card_ticket = $json['ticket'];
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
            $this->setCache($authname, $this->card_ticket, $expire);
            return $this->card_ticket;
        }
        return false;
    }
    ...
    //上边的生成jssign的方法不对，试试下边这个
    /**
     * 卡券塞入卡包调用签名(高祥栋 2015-4-2 add)
     * @param string $card_id 生成卡券时获得的card_id
     * @param string $code 指定的卡券code码，只能被领一次。use_custom_code字段为true的卡券必须填写，非自定义code不必填写。
     * @param string $openid 指定领取者的openid，只有该用户能领取。bind_openid字段为true的卡券必须填写，非自定义openid不必填写。
     * @param string $timestamp 当前时间戳 (为空则自动生成)
     * @param string $outer_id 场景参数值，整形。领取的事件推送里新增OuterId字段，会推送CardExt里面的outer_id字段，默认推送0。长度限制在14个数字内，不支持字母。
     * @param string $balance 红包余额，以分为单位。红包类型必填（LUCKY_MONEY），其他卡券类型不填。
     * @param string $appid 用于多个appid时使用,可空
     * @return array|bool 返回签名字串
     */
    public function getJsSign2($card_id, $code = '', $openid = '', $timestamp = 0, $outer_id = 0, $balance = '', $appid = '') {
        if (!$this->jsapi_ticket && !$this->getCardTicket($appid))
            return false;
        if (!$timestamp)
            $timestamp = time() . ''; //接口要求是字符串
        $signature = new Signature();
        $signature->add_data($this->jsapi_ticket);
        ...
        if ($balance)
            $signature->add_data($balance);
        $sign['code'] = $code;
        $sign['openid'] = $openid;
        $sign['timestamp'] = $timestamp;
        $sign['signature'] = $signature->get_signature();
        $sign['outer_id'] = $outer_id;
        if ($balance)
    ...
    //上边的生成jssign的方法不对，试试下边这个
    /**
     * 拉起卡券列表调用签名(高祥栋 2015-4-2 add)
     * @param string $card_id 生成卡券时获得的card_id
     * @param string $timestamp 当前时间戳 (为空则自动生成)
     * @param string $noncestr 随机串 (为空则自动生成)
     * @param string $appid 用于多个appid时使用,可空
     * @param string $card_type 卡券类型,可空
     * @param string $location_id 门店信息,可空
     * @return array|bool 返回签名字串
     */
    public function getJsSign3($card_id = '', $timestamp = 0, $noncestr = '', $appid = '', $card_type = '', $location_id = '') {
        if (!$this->jsapi_ticket && !$this->getCardTicket($appid))
            return false;
        if (!$timestamp)
            $timestamp = time();
        ...
        if (!empty($card_type)) {
            $signature->add_data($card_type);
        }
        if (!empty($location_id)) {
            $signature->add_data($location_id);
        }

        $sign['app_id'] = $this->appid;
        $sign['location_id'] = $location_id;
        $sign['card_id'] = $card_id;
        $sign['card_type'] = $card_type;
        $sign['nonce_str'] = $noncestr;
        $sign['timestamp'] = $timestamp;
        $sign['cardsign'] = $signature->get_signature();
        return $sign;
    }
...
// :TODO: Is there any more datatype we should be in the lookout for? (Object?)
    ...
    public function uploadMedia($data, $type) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::UPLOAD_MATERIAL_URL . self::MEDIA_UPLOAD_URL . 'access_token=' . $this->access_token . '&type=' . $type, $data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function uploadMaterial($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::UPLOAD_MATERIAL_URL . self::MATERIAL_ADDMATERIAL_URL . 'access_token=' . $this->access_token, $data, true);
    ...
    /**
     * 根据媒体文件ID获取临时媒体文件(认证后的订阅号可用)（2015-4-6 高祥栋 update）
     * @param string $media_id 媒体文件id
     * @return raw data
     */
    public function getMedia($media_id, $type='other') {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        if ($type == 'video') {
            $result = $this->http_get(self::UPLOAD_MATERIAL_URL . self::MEDIA_GET_URL . 'access_token=' . $this->access_token . '&media_id=' . $media_id);
        } else {
    ...
    /**
     * 根据媒体文件ID获取永久媒体文件(认证后的订阅号可用)（2015-4-6 高祥栋 add）
     * @param array $data
     * @return raw data
     */
    public function getMaterial($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::MATERIAL_GETMATERIAL_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 根据媒体文件ID删除永久媒体文件(认证后的订阅号可用)（2015-4-6 高祥栋 add）
     * @param array $data
     * @return raw data
     */
    public function delMaterial($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::MATERIAL_DELMATERIAL_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 上传临时图文消息素材(好像过期了已经  高祥栋 2015-4-6 update)
    ...
    /**
     * 上传永久图文消息素材(好像过期了已经  高祥栋 2015-4-6 add)
     * @param array $data 消息结构{"articles":[{...}]}
     * @return boolean|array
     */
    public function uploadArticles2($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::MATERIAL_ADDNEWS_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function updateArticles($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::MATERIAL_UPDATENEWS_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @param int $type 0:临时二维码；1:永久二维码(此时expire参数无效：永久数字)；2:永久二维码(此时expire参数无效:永久字符串)
    ...
    /**
     * 上传门店logo
     * @param Array $data   文件数据流
     * @return array|boolean 返回数组中url为图片地址
     */
    public function uploadLogo($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::CARD_UPLOAD_LOGO . 'access_token=' . $this->access_token, $data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @param boolean $is_unique_code 指定下发二维码，生成的二维码随机分配一个 code，领取后不可再次扫描。填写 true 或 false。默认 false。【测试了一下，当为true时，测试失败】
     * @param string $balance 红包余额，以分为单位。红包类型必填（LUCKY_MONEY），其他卡券类型不填。
     * @param string $outer_id 场景值，用于领取渠道的数据统计，默认值为0，字段类型为整型。用户领取卡券后触发的事件推送中会带上此自定义场景值。
     * @return boolean|string
     */
    ...
    /**
     * 更新电影票(高祥栋 2015-4-4 add)
     * 领取电影票后通过调用“更新电影票”接口update电影信息及用户选座信息
     * @param string $data 具体结构请参看卡券开发文档(6.1.2 会员卡交易)章节
     * @return boolean|array
     */
    public function updateMovieTicket($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MOVIETICKET_UPDATEUSER . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 飞机票-在线选座(高祥栋 2015-4-4 add)
     * @param string $data 具体结构请参看卡券开发文档(6.1.2 会员卡交易)章节
     * @return boolean|array
     */
    public function boardingpassCheckin($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_BOARDINGPASS_CHECKIN . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 更新会议门票(高祥栋 2015-4-4 add)
     * @param string $data 具体结构请参看卡券开发文档(6.1.2 会员卡交易)章节
     * @return boolean|array
     */
    public function updateMeetingTicket($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEETINGTICKET_UPDATEUSER . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
